...
alpha , beta , gamma = [0.2,0.2,0.2,0.2],[2,2,2,2],[0.05,0.05,0.05,0.05]
...
for i in range(4):
    f = fat[i]
    my_world.add_dissease(alpha,beta,gamma,"Chicken Pox",f) # adds the disease parameters
    sol_SIR,t = my_world.forcast_SIR(initial_conditions_SIR,T_final,dt,k=i)
    leg[i],=plt.plot(t,sol_SIR[:,1]/Nations[i],label = f'$\\mu$= {f}',linestyle='-',color=(Color[i][0],Color[i][1],Color[i][2]) )
...
plt.ylim(0,1.4)
plt.savefig("SIR_fatality",dpi = 300,transparant = True)
plt.show()
