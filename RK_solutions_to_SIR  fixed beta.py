...
alpha , beta , gamma = [0.04,0.08,0.12,0.16],[0.7,0.7,0.7,0.7],[0,0,0,0]
Nations = [10000,10000,10000,10000]
initial_conditions_SIR = [9800,200,0]
...
for i in range(4):
    ...
    leg[i],=plt.plot(t,sol_SIR[:,1]/Nations[i],label = f'$\\alpha$= {alpha[i]}',linestyle='-',color=(Color[i][0],Color[i][1],Color[i][2]) )
...
plt.title(f'Runge Kutta 4 solutions to the SIR-model for $\\beta$ = 0.7')
plt.ylim(0,1.4)
plt.savefig("SIR fixed beta",dpi = 300,transparent = True)
plt.show()
